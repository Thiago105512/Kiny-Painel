...
(async () => {
  ...
  console.log('1) Lista');
  ok(await p.locator('.jg-cartao').count() === 5, '5 jogos na lista (sem objetivo definido)');
  console.log('2) Três vidas: errar 3 vezes encerra');
  await p.goto(URL + '#/jogos/vidas'); await p.waitForTimeout(300); await p.click('[data-act="jg-comecar"]'); await p.waitForTimeout(200);
  ...
  await p.goto(URL + '#/jogos/relogio'); await p.waitForTimeout(300); await p.click('[data-act="jg-comecar"]'); await p.waitForTimeout(200);
  ...
  await p.goto(URL + '#/jogos/vf'); await p.waitForTimeout(300); await p.click('[data-act="jg-comecar"]'); await p.waitForTimeout(200);
  ...
  await p.goto(URL + '#/jogos/ecg'); await p.waitForTimeout(300); await p.click('[data-act="jg-comecar"]'); await p.waitForTimeout(200);
  ...
  await p.goto(URL + '#/jogos/linha'); await p.waitForTimeout(300); await p.click('[data-act="jg-comecar"]'); await p.waitForTimeout(200);
  ...
  ok(!r.l.includes('ecg') && !r.med, 'objetivo Direito: sem jogo de ECG e sem questões de Medicina (' + r.l.join(',') + ')');
