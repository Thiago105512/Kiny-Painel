...
await go('#/revisoes/erros');ok((await txt()).includes('Refazer lote'),'erros agrupados em lotes por tema');await go('#/revisoes/erros/todos');ok(!!(await p.$('[data-act="erros-iniciar"]')),'lote só começa pelo botão Começar');await p.click('[data-act="erros-iniciar"]');await p.waitForTimeout(150);await responder(true);
