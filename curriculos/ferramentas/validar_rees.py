...
if pos and max(pos.values()) - min(pos.get(i, 0) for i in range(5)) > max(3, n // 8): E.append(f"posição da correta desequilibrada: {dict(pos)}")
...
for i, nome in enumerate(["curtas (<250)", "médias (250–599)", "longas (≥600)"]):
    ...
    if not lo - 0.02 <= fr <= hi + 0.02: E.append(f"proporção de enunciados {nome}: {round(fr*100)}% fora do alvo {int(lo*100)}–{int(hi*100)}%")
# nível e tamanho não são a mesma coisa: exige questões difíceis curtas e fáceis longas também, sem ser regra fixa
if not any(c == 0 and q.get("dificuldade") == 3 for c, q in zip(cls, novo)) and len(novo) >= 40: E.append("inclua ao menos uma questão curta e difícil (tamanho não define dificuldade)")
